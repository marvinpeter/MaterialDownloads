...
export const DownloadItem = ({ item, anchorEl, handleMenuOpen, handleMenuClose, show, open, resume, pause, remove, retry, acceptDanger, copyLink, cancel }: DownloadItemProps) => (
    <ListItem draggable={item.status === DownloadStatus.Completed} onDragStart={e => { e.preventDefault(); chrome.downloads.drag(item.id) }} dense>
        ...
        <DownloadItemStatus {...{ item, show, open, resume, pause, remove, retry, acceptDanger }} />
